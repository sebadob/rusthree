...
pub trait Request {
    ...
    fn request_body(&self) -> Vec<u8> {
        ...
        if let Command::PutObject { content, .. } = command {
            Vec::from(*content)
    ...
    fn string_to_sign(&self, request: &str) -> Result<String, S3Error> {
        match self.command() {
            Command::PresignPost { post_policy, .. } => Ok(post_policy.clone()),
    ...
    fn url(&self) -> Result<Url, S3Error> {
        ...
        if let Command::CreateBucket { .. } = command {
    ...
    async fn headers(&self) -> Result<HeaderMap, S3Error> {
        ...
        let command = self.command();
        if let Command::PutObjectTagging { tags } = command {
        ...
        } else if let Command::PutObject { content, .. } = command {
            ...
            let hash = general_purpose::STANDARD.encode(digest.as_ref());
            headers.insert(HeaderName::from_static("content-md5"), hash.parse()?);
        } else if let Command::GetObject {} = command {
        ...
        } else if let Command::GetObjectRange { start, end } = command {
            headers.insert(ACCEPT, "application/octet-stream".to_string().parse()?);
        ...
        } else if let Command::CreateBucket { ref config } = command {
